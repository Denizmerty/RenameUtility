...
// Ensures 'input' is treated as a literal string within a regex pattern by escaping metacharacters
...
// Converts a filename wildcard pattern (using '*' and '?') into its equivalent regular expression
...
// Extracts the last integer found in 'filename', useful for filtering or sequence manipulation
std::optional<int> RenamerLogic::ParseLastNumber(const std::string &filename)
{
    ...
    if (std::regex_match(filename, match, last_number_regex))
    {
        if (match.size() == 2) // Ensures the number group was captured
        {
            try
            {
                // Use long long for intermediate conversion to detect overflow against int limits
                long long num_ll = std::stoll(match[1].str());
                if (num_ll >= std::numeric_limits<int>::min() && num_ll <= std::numeric_limits<int>::max())
                {
...
// Replaces all occurrences of 'find' with 'replace' in 'subject', respecting 'caseSensitive' flag
std::string RenamerLogic::PerformFindReplace(std::string subject, const std::string &find,
                                             const std::string &replace, bool caseSensitive)
{
    ...
    while (pos < subject.length())
    {
        ...
        {
            auto it = std::search(subject.begin() + pos, subject.end(),
                                  find.begin(), find.end(),
                                  [](unsigned char c1, unsigned char c2)
                                  {
                                      return std::tolower(c1) == std::tolower(c2);
                                  });
            found_pos = (it == subject.end()) ? std::string::npos : static_cast<size_t>(it - subject.begin());
        }
        ...
        // Advance position past the replacement to prevent infinite loops with overlapping patterns
        pos = found_pos + replace.length();
    }
    return subject;
}

// Constructs new filenames by substituting placeholders in 'pattern' with runtime values
std::string RenamerLogic::ReplacePlaceholders(
    const std::string &pattern, RenamingMode mode, int index, int totalManualFiles,
    const std::string &originalFullName,
    const std::string &originalNameStem,
    const std::string &originalExtension,
    const std::optional<int> &dirScanOriginalNum,
    const std::optional<int> &dirScanNewNum,
    int dirScanNumberWidth)
{
    std::string result = pattern;
    size_t pos;

    // Efficiently check for and replace date/time placeholders if any are present
    if (result.find("<YYYY>") != std::string::npos || result.find("<MM>") != std::string::npos || /* ... other time placeholders ... */
        result.find("<DD>") != std::string::npos || result.find("<hh>") != std::string::npos ||
        result.find("<mm>") != std::string::npos || result.find("<ss>") != std::string::npos)
    {
            ...
            [&](const std::string &placeholder, const char *format)
        {
            ...
            while (current_pos != std::string::npos)
            {
                ...
                current_pos = result.find(placeholder, current_pos + tempStr.length());
            }
        };
    ...
    if (mode == RenamingMode::DirectoryScan)
    {
        ...
        std::string newNumStr = dirScanNewNum.has_value() ? FormatNumber(dirScanNewNum.value(), dirScanNumberWidth) : "";
        pos = result.find("<num>");
        while (pos != std::string::npos)
        {
        ...
        std::string origNumStr = dirScanOriginalNum.has_value() ? FormatNumber(dirScanOriginalNum.value(), dirScanNumberWidth) : "";
    ...
    else // RenamingMode::ManualSelection
    {
        ...
        int indexWidth = (totalManualFiles > 0) ? static_cast<int>(std::floor(std::log10(totalManualFiles))) + 1 : 1;
    ...
    // Correctly identify stem vs extension (e.g. ".bashrc" has no stem for this logic)
    if (last_dot_pos != std::string::npos && last_dot_pos != 0 && last_dot_pos + 1 < result.size())
    {
    ...
    if ((sanitized_stem == "_" && preserved_ext.empty()) || (sanitized_stem.empty() && preserved_ext.empty()))
    {
        return "_";
    }
...
// Modifies the case of the filename's stem (part before extension) according to 'mode'
std::string RenamerLogic::ApplyCaseConversion(std::string filename, CaseConversionMode mode)
{
    ...
    if (!filename.empty() && filename[0] == '.' && p.stem().string() == filename)
    {
        return filename;
    }
