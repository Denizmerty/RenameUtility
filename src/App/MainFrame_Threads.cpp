...
void MainFrame::OnPreviewThreadComplete(wxCommandEvent &event)
{
	...
	if (!results)
	{
		...
		logTextCtrl->AppendText("Error: Failed to receive preview results from worker thread.\n");
		logTextCtrl->SetDefaultStyle(normalStyle);
	...
	{
		logTextCtrl->AppendText("Potential Overwrite: Skipped renaming '" + wxString(po.SourceFile) + "' to '" + wxString(po.TargetFile) + "' because target path exists and is not part of this rename batch.\n");
	}
	logTextCtrl->SetDefaultStyle(redStyle);
	...
	// Clear the preview list (and any associated item data from Manual mode's pre-population) before repopulating with results
	...
	{
		logTextCtrl->AppendText("Populating preview list with planned renames...\n");
		long itemIndex = 0;
		for (const auto &op : m_lastPreviewResults.renamePlan)
		{
			...
			{																  // Manual Mode
				previewList->InsertItem(itemIndex, std::to_string(op.Index)); // Use 1-based index from the operation
				previewList->SetItem(itemIndex, 1, wxString(op.OldName));
				previewList->SetItem(itemIndex, 2, wxString(op.NewName));
				// Store a new fs::path pointer for the OldFullPath of items in the actual rename plan
				// This replaces any path data stored during the initial PopulateManualPreviewList
				previewList->SetItemData(itemIndex, reinterpret_cast<wxUIntPtr>(new fs::path(op.OldFullPath)));
			}
			itemIndex++;
		}

		// Enable/Disable rename button based on overall success and if there are items in the plan
		if (m_previewSuccess)
		{
			logTextCtrl->AppendText("Preview generated successfully.\n");
			UpdateStatusBar(wxString::Format("Preview ready: %d file(s) to be renamed.", (int)m_lastPreviewResults.renamePlan.size()));
			renameButton->Enable(true);
		}
		else
		{
			logTextCtrl->AppendText("Preview generation completed with errors. See log.\n");
			UpdateStatusBar("Preview failed. Check log.");
			renameButton->Enable(false);
			wxMessageBox("Preview generation failed or encountered errors. Please check the log for details.",
						 "Preview Error", wxOK | wxICON_ERROR, this);
		}
	}
	else
	{								 // No items in the renamePlan
		renameButton->Enable(false); // Ensure rename is disabled
		if (m_previewSuccess)
		{
			// Preview technically succeeded, but no files matched criteria or were eligible
			logTextCtrl->AppendText("Preview complete: No files found matching criteria or eligible for renaming.\n");
			UpdateStatusBar("Preview: No files eligible for rename.");
			// Show an info box only if there were no errors/warnings at all, otherwise a warning
			if (m_lastPreviewResults.errorLog.empty() && m_lastPreviewResults.warningLog.empty() && m_lastPreviewResults.missingSourceFilesLog.empty() && m_lastPreviewResults.potentialOverwritesLog.empty())
			{
				wxMessageBox("No files were found matching the specified criteria or no renames are necessary.",
							 "Preview Information", wxOK | wxICON_INFORMATION, this);
			}
			else
			{
				wxMessageBox("Preview complete, but no files are eligible for renaming. Check the log for skipped files or warnings.",
		...
		else
		{
			...
			logTextCtrl->AppendText("Preview generation failed, resulting in no files to rename. See log.\n");
			UpdateStatusBar("Preview failed. Check log.");
			wxMessageBox("Preview generation failed. Please check the log for errors.",
						 "Preview Error", wxOK | wxICON_ERROR, this);
		}
	}
}
...
void MainFrame::OnRenameThreadComplete(wxCommandEvent &event)
{
	...
	RenameThreadResults *results = static_cast<RenameThreadResults *>(event.GetClientData());
	if (!results)
	{
		logTextCtrl->SetDefaultStyle(redStyle);
		logTextCtrl->AppendText("Error: Failed to receive rename results from worker thread.\n");
		logTextCtrl->SetDefaultStyle(normalStyle);
	...
	m_lastBackupPath = m_lastBackupResult.success ? m_lastBackupResult.backupPath : fs::path();

	delete results; // Delete heap-allocated data

	// Process backup results first. If backup failed, rename was aborted by the worker
	if (m_backupAttempted)
	{
		if (!m_lastBackupResult.success)
		{
			logTextCtrl->SetDefaultStyle(redStyle);
			logTextCtrl->AppendText("CRITICAL ERROR: Backup failed: " + wxString(m_lastBackupResult.errorMessage) + "\nRename operation was aborted.\n");
			logTextCtrl->SetDefaultStyle(normalStyle);
			UpdateStatusBar("Backup failed. Rename aborted.");
			wxMessageBox("Backup FAILED!\n" + m_lastBackupResult.errorMessage + "\nThe rename operation was aborted to prevent data loss.",
						 "Backup Error", wxOK | wxICON_ERROR, this);
			m_previewSuccess = false;
			renameButton->Enable(false);
			SetUndoAvailable(false);
			// Clear list items and associated data (especially in Manual mode) as state is now invalid
			for (long i = 0; i < previewList->GetItemCount(); ++i)
			{
				wxUIntPtr data = previewList->GetItemData(i);
				if (data)
		...
		{
			logTextCtrl->AppendText("Backup completed successfully: " + wxString(m_lastBackupPath.wstring()) + "\n");
		}
	}

	// Process rename results
	bool renameAttempted = !m_lastRenameResult.successfulRenameOps.empty() || !m_lastRenameResult.failedRenames.empty();
	...
	if (renameAttempted)
	{
		...
		{
			logTextCtrl->AppendText("Success: '" + wxString(op.OldName) + "' renamed to '" + wxString(op.NewName) + "'\n");
		}
		// Log failed renames
		if (failCount > 0)
		{
			...
			{
				logTextCtrl->AppendText("FAILED: '" + wxString(pair.first) + "': " + wxString(pair.second) + "\n");
			}
			logTextCtrl->SetDefaultStyle(normalStyle);
		}
		...
		if (m_lastRenameResult.overallSuccess)
		{
			...
			UpdateStatusBar(wxString::Format("Rename successful: %d file(s) renamed.", successCount));
			wxMessageBox(wxString::Format("%d file(s) renamed successfully.", successCount),
						 "Rename Successful", wxOK | wxICON_INFORMATION, this);
			// Enable Undo only if the rename was fully successful and resulted in changes
			m_undoableOperations = m_lastRenameResult.successfulRenameOps;
			SetUndoAvailable(!m_undoableOperations.empty());
		}
		else
		{
			logTextCtrl->AppendText("Rename operation completed with errors.\n");
			UpdateStatusBar(wxString::Format("Rename finished: %d successful, %d failed.", successCount, failCount));
			wxMessageBox(wxString::Format("Rename operation completed, but %d error(s) occurred. Please check the log.", failCount),
	...
	{
		// No rename operations were attempted (e.g., backup failed, or plan was initially empty)
		if (m_backupAttempted && m_lastBackupResult.success)
		{
			logTextCtrl->AppendText("Backup was successful, but no rename operations were performed.\n");
	...
	// If in manual mode, clear the internal list as files have been renamed (or failed)
	if (m_currentMode == RenamingMode::ManualSelection)
	{
		m_manualFiles.clear();
		PopulateManualPreviewList(); // Update UI to reflect empty list
	...
	// Handle backup retention messaging based on rename success and undo availability
	bool renameSucceededFully = renameAttempted && m_lastRenameResult.overallSuccess;
	if (m_backupAttempted && m_lastBackupResult.success && !m_lastBackupPath.empty())
	{
		if (renameSucceededFully && m_undoAvailable)
		{
			// Successful rename, undo is possible, so backup is key for full recovery
			logTextCtrl->AppendText("Backup retained at: " + wxString(m_lastBackupPath.wstring()) + "\n");
			UpdateStatusBar(wxString::Format("Finished: %d OK. Backup kept.", successCount));
		}
		else if (!renameSucceededFully)
		{
			// Rename had errors, backup is essential for potential manual recovery
			logTextCtrl->AppendText("Backup retained due to rename errors: " + wxString(m_lastBackupPath.wstring()) + "\n");
			wxMessageBox("Rename operation had errors. The backup has been retained:\n" + m_lastBackupPath.wstring(),
						 "Backup Retained", wxOK | wxICON_WARNING, this);
			UpdateStatusBar(wxString::Format("Finished: %d OK, %d FAIL. Backup retained.", successCount, failCount));
		}
	}
}
...
void MainFrame::OnUndoThreadComplete(wxCommandEvent &event)
{
	...
	{
		logTextCtrl->SetDefaultStyle(redStyle);
		logTextCtrl->AppendText("Error: Failed to receive undo results from worker thread.\n");
		logTextCtrl->SetDefaultStyle(normalStyle);
	...
	{
		logTextCtrl->AppendText("Success: Reverted '" + wxString(pair.first) + "' back to '" + wxString(pair.second) + "'\n");
	}
	if (!results->failedUndos.empty())
	{
		...
		{
			logTextCtrl->AppendText("FAILED Undo: '" + wxString(pair.first) + "': " + wxString(pair.second) + "\n");
		}
		logTextCtrl->SetDefaultStyle(normalStyle);
	}
	...
	if (results->overallSuccess)
	{
		...
		UpdateStatusBar(wxString::Format("Undo successful: %zu file(s) reverted.", results->successfulUndos.size()));
		wxMessageBox("The last rename operation was successfully undone.", "Undo Complete", wxOK | wxICON_INFORMATION, this);
	}
	else
	{
		logTextCtrl->AppendText("Undo operation completed with errors.\n");
		UpdateStatusBar(wxString::Format("Undo finished: %zu successful, %zu failed.", results->successfulUndos.size(), results->failedUndos.size()));
		wxMessageBox(wxString::Format("Undo operation completed, but %zu error(s) occurred. Please check the log and verify the file status manually.", results->failedUndos.size()),
	...
	// The last backup (m_lastBackupResult, m_lastBackupPath) might still be relevant for user reference

	// If in manual mode, clear the list as file state is potentially mixed after undo
	if (m_currentMode == RenamingMode::ManualSelection)
	{
		m_manualFiles.clear();
		PopulateManualPreviewList(); // Update UI to reflect empty list
