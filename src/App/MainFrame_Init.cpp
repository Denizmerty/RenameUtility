...
// MainFrame constructor: Initializes UI elements, menus, status bar, and loads settings
// MainFrame constructor: Initializes UI elements, menus, status bar, and loads settings
MainFrame::MainFrame(const wxString &title, const wxPoint &pos, const wxSize &size)
	: wxFrame(NULL, wxID_ANY, title, pos, size),
	  m_currentMode(RenamingMode::DirectoryScan), // Default mode is DirectoryScan
	  ...
	  m_backupAttempted(false)
{
	...
	menuFile->Append(ID_SaveProfile, "Save Profile...");
	menuFile->Append(ID_LoadProfile, "Load Profile...");
	menuFile->Append(ID_DeleteProfile, "Delete Profile...");
	menuFile->AppendSeparator();
	menuFile->Append(ID_UndoRename, "Undo Last Rename\tCtrl+Z"); // Add accelerator hint
	menuFile->AppendSeparator();
	...
	// Create UI controls
	scrolledWindow = new wxScrolledWindow(mainPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVSCROLL | wxBORDER_SUNKEN);
	wxArrayString modeChoices;
	modeChoices.Add("Directory Scan");
	modeChoices.Add("Manual File Selection");
	modeSelectionRadio = new wxRadioBox(scrolledWindow, ID_ModeSelectionRadio, "Operation Mode", wxDefaultPosition, wxDefaultSize, modeChoices, 1, wxRA_SPECIFY_COLS);
	modeSelectionRadio->SetSelection(0); // Default to "Directory Scan"
	dirScanBox = new wxStaticBox(scrolledWindow, wxID_ANY, "Directory Scan Options");
	manualBox = new wxStaticBox(scrolledWindow, wxID_ANY, "Manual File Selection Options");
	commonBox = new wxStaticBox(scrolledWindow, wxID_ANY, "Common Renaming Options");
	dirPicker = new wxDirPickerCtrl(scrolledWindow, ID_DirPicker, wxEmptyString, "Select...", wxDefaultPosition, wxDefaultSize, wxDIRP_DEFAULT_STYLE | wxDIRP_DIR_MUST_EXIST);
	fileNamePatternLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Filename Pattern (find, uses *, ?):");
	fileNamePatternCtrl = new wxTextCtrl(scrolledWindow, ID_FileNamePatternCtrl, "*.*");
	filterExtensionsLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Filter by Extensions (opt., comma-sep):");
	filterExtensionsCtrl = new wxTextCtrl(scrolledWindow, ID_FilterExtensionsCtrl, "");
	lowestNumLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Lowest Number (optional filter):");
	lowestNumSpin = new wxSpinCtrl(scrolledWindow, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 9999, 0);
	highestNumLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Highest Number (optional filter):");
	highestNumSpin = new wxSpinCtrl(scrolledWindow, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 9999, 0);
	recursiveCheck = new wxCheckBox(scrolledWindow, ID_RecursiveCheck, "Include Subdirectories");
	addFilesButton = new wxButton(scrolledWindow, ID_AddFilesButton, "Add Files...");
	removeFilesButton = new wxButton(scrolledWindow, ID_RemoveFilesButton, "Remove Selected");
	removeFilesButton->Enable(false); // Initially disabled as list is empty
	clearFilesButton = new wxButton(scrolledWindow, ID_ClearFilesButton, "Clear List");
	clearFilesButton->Enable(false); // Initially disabled
	patternLabel = new wxStaticText(scrolledWindow, wxID_ANY, "New Naming Pattern:");
	patternCtrl = new wxTextCtrl(scrolledWindow, ID_PatternCtrl, "<orig_name><ext>");
	findLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Find Text (Optional):");
	...
	caseSensitiveCheck = new wxCheckBox(scrolledWindow, ID_CaseCheck, "Case Sensitive Find");
	...
	caseChoice = new wxChoice(scrolledWindow, ID_CaseChoice, wxDefaultPosition, wxDefaultSize, caseOptions);
	caseChoice->SetSelection(0); // Default to "No Change"
	incrementLabel = new wxStaticText(scrolledWindow, wxID_ANY, "Increment By:");
	incrementSpin = new wxSpinCtrl(scrolledWindow, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, -9999, 9999, 1);
	backupCheck = new wxCheckBox(scrolledWindow, wxID_ANY, "Create backup before renaming");
	bottomPanel = new wxPanel(mainPanel, wxID_ANY); // Panel for buttons, preview list, and log
	...
	previewList = new wxListCtrl(bottomPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL);
	logLabel = new wxStaticText(bottomPanel, wxID_ANY, "Log:");
	logTextCtrl = new wxTextCtrl(bottomPanel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY | wxTE_RICH2);
	m_defaultTextCtrlBgColour = fileNamePatternCtrl->GetBackgroundColour(); // Store default for resetting error states
	...
	// Finalize window sizing, respecting minimum requirements and saved dimensions
	...
	// Ensure saved dimensions are not smaller than minimum required or arbitrary minimums
...
{
	// Sizer for the top input area (mode selection, scan options, manual options, common options)
	...
	wxFlexGridSizer *dirGridSizer = new wxFlexGridSizer(5, 2, 5, 5); // 5 rows, 2 columns, 5px gaps
	dirGridSizer->AddGrowableCol(1);								 // Second column (controls) should grow
	dirGridSizer->Add(new wxStaticText(scrolledWindow, wxID_ANY, "Target Directory:"), 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	dirGridSizer->Add(dirPicker, 1, wxEXPAND | wxALL, 2);
	dirGridSizer->Add(fileNamePatternLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	dirGridSizer->Add(fileNamePatternCtrl, 1, wxEXPAND | wxALL, 2);
	dirGridSizer->Add(filterExtensionsLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	dirGridSizer->Add(filterExtensionsCtrl, 1, wxEXPAND | wxALL, 2);
	dirGridSizer->Add(lowestNumLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	dirGridSizer->Add(lowestNumSpin, 1, wxEXPAND | wxALL, 2);
	dirGridSizer->Add(highestNumLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	dirGridSizer->Add(highestNumSpin, 1, wxEXPAND | wxALL, 2);
	dirScanSizer->Add(dirGridSizer, 0, wxEXPAND | wxALL, 5);
	dirScanSizer->Add(recursiveCheck, 0, wxALIGN_LEFT | wxLEFT | wxRIGHT | wxBOTTOM, 5);
	inputAreaSizer->Add(dirScanSizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);
	...
	manualButtonSizer->AddStretchSpacer(1); // Pushes Remove/Clear buttons to the right
	...
	inputAreaSizer->Add(manualSizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);

	// Sizer for Common Renaming Options
	commonSizer = new wxStaticBoxSizer(commonBox, wxVERTICAL);
	wxFlexGridSizer *commonGridSizer = new wxFlexGridSizer(6, 2, 5, 5); // 6 rows, 2 columns
	commonGridSizer->AddGrowableCol(1);									// Second column (controls) grows
	commonGridSizer->Add(patternLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	commonGridSizer->Add(patternCtrl, 1, wxEXPAND | wxALL, 2);
	commonGridSizer->Add(findLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	commonGridSizer->Add(findCtrl, 1, wxEXPAND | wxALL, 2);
	commonGridSizer->Add(replaceLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	...
	commonGridSizer->Add(caseSensitiveCheck, 1, wxEXPAND | wxALL, 2);
	commonGridSizer->Add(caseChoiceLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	commonGridSizer->Add(caseChoice, 1, wxEXPAND | wxALL, 2);
	commonGridSizer->Add(incrementLabel, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 2);
	commonGridSizer->Add(incrementSpin, 1, wxEXPAND | wxALL, 2);
	commonSizer->Add(commonGridSizer, 0, wxEXPAND | wxALL, 5);
	inputAreaSizer->Add(commonSizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);

	inputAreaSizer->Add(backupCheck, 0, wxALIGN_LEFT | wxLEFT | wxRIGHT | wxBOTTOM, 10);
	...
	bottomAreaSizer->Add(actionButtonSizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);

	// Sizer to hold preview list and log side-by-side
	wxBoxSizer *previewLogSizer = new wxBoxSizer(wxHORIZONTAL);
	previewLogSizer->Add(previewList, 1, wxEXPAND | wxALL, 5); // Preview list takes half the space
	...
	previewLogSizer->Add(logAreaSizer, 1, wxEXPAND | wxALL, 5);							 // Log area takes the other half
	bottomAreaSizer->Add(previewLogSizer, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5); // Preview/Log area expands
	bottomPanel->SetSizer(bottomAreaSizer);

	// Main sizer for the frame, dividing space between scrolled input area and bottom panel
	wxBoxSizer *mainFrameSizer = new wxBoxSizer(wxVERTICAL);
	mainFrameSizer->Add(scrolledWindow, 1, wxEXPAND | wxALL, 5); // Input area expands
	mainFrameSizer->Add(bottomPanel, 1, wxEXPAND | wxALL, 5);	 // Bottom panel also expands
...
void MainFrame::BindEvents()
{
	...
	Bind(wxEVT_BUTTON, &MainFrame::OnRemoveFilesClick, this, ID_RemoveFilesButton);
	Bind(wxEVT_BUTTON, &MainFrame::OnClearFilesClick, this, ID_ClearFilesButton);
	previewButton->Bind(wxEVT_BUTTON, &MainFrame::OnPreviewClick, this, ID_PreviewButton);
	renameButton->Bind(wxEVT_BUTTON, &MainFrame::OnRenameClick, this, ID_RenameButton);
